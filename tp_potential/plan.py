...
def make_start_plan_non_degenerate(x: np.ndarray) -> None:
    """
    Modify the matrix x in place to ensure a non-degenerate solution.
    
    Args:
        x (np.ndarray): The transportation problem solution matrix.
    """
    m, n = x.shape

    # Ensure that the number of non-zero cells equals m + n - 1
    MAX_ITER = 100
    iter = 0
    while np.count_nonzero(x) != m + n - 1 and iter < MAX_ITER:
        iter += 1
        for i in range(m):
            if np.count_nonzero(x[i]) == 1:
                # Find the column index of the non-zero element in the row i
                j = np.nonzero(x[i])[0][0]

                # Check if column j also has only one non-zero element
                if np.count_nonzero(x[:, j]) == 1:
                    if np.nonzero(x[:, j])[0][0] == i:
                        # Add an artificial zero (np.nan) to an adjacent cell
                        if i < m - 1:
                            x[i + 1][j] = np.nan
                        else:
                            x[i - 1][j] = np.nan
